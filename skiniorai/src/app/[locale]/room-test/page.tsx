...
export default function RoomTestPage() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 ${isRTL ? 'rtl font-cairo' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className={`text-3xl font-bold text-gray-900 mb-2 ${isRTL ? 'font-cairo' : ''}`}>
              {isRTL ? 'استشارة العناية بالبشرة بالذكاء الاصطناعي' : 'AI Skincare Consultation'}
            </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                    devicePermissions.testing ? 'bg-blue-100' : 
                  ...
                  <span className={`${isRTL ? 'font-cairo' : ''} ${
                    devicePermissions.testing ? 'text-blue-600' :
                    devicePermissions.camera ? 'text-green-700' : 'text-red-600'
                  }`}>
                    {devicePermissions.testing ? 
                      (isRTL ? 'جاري اختبار الكاميرا...' : 'Testing Camera...') :
                      devicePermissions.camera ? 
                        (isRTL ? 'الكاميرا متاحة' : 'Camera Available') :
                        (isRTL ? 'الكاميرا غير متاحة' : 'Camera Not Available')
                    }
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`${isRTL ? 'font-cairo' : ''} ${
                    devicePermissions.testing ? 'text-blue-600' :
                    devicePermissions.microphone ? 'text-green-700' : 'text-red-600'
                  }`}>
                    {devicePermissions.testing ? 
                      (isRTL ? 'جاري اختبار الميكروفون...' : 'Testing Microphone...') :
                      devicePermissions.microphone ? 
                        (isRTL ? 'الميكروفون متاح' : 'Microphone Available') :
                        (isRTL ? 'الميكروفون غير متاح' : 'Microphone Not Available')
                    }
                  </span>
                </div>

                {!devicePermissions.testing && (!devicePermissions.camera || !devicePermissions.microphone) && (
                  <div className={`mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg ${isRTL ? 'text-right' : ''}`}>
                    <p className={`text-sm text-yellow-800 ${isRTL ? 'font-cairo' : ''}`}>
                      {isRTL 
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div className={`space-y-3 text-sm text-gray-600 ${isRTL ? 'font-cairo text-right' : ''}`}>
                <p>
                  {isRTL 
              ...
              <label className="flex items-start gap-3 mt-6 cursor-pointer">
                ...
                />
                <span className={`text-sm text-gray-700 ${isRTL ? 'font-cairo text-right' : ''}`}>
                  {isRTL 
          ...
          <div className="text-center mt-8">
            ...
            {(!hasAcceptedTerms || !devicePermissions.camera || !devicePermissions.microphone) && (
              <p className={`mt-2 text-sm text-gray-500 ${isRTL ? 'font-cairo' : ''}`}>
                {isRTL 
