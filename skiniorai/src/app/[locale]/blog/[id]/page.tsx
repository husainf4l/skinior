...
import { BlogErrorWrapper, BlogPostErrorFallback } from "../../../../components/blog/BlogErrorBoundary";
import BlogSkeleton from "../../../../components/blog/BlogSkeleton";
import ReadingProgress, { ReadingStats } from "../../../../components/blog/ReadingProgress";

import EnhancedComments from "../../../../components/blog/EnhancedComments";
import { sanitizeHTML, validateBlogContent } from "../../../../utils/contentSanitizer";
...
export default function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  // Get text by locale
  const getText = useCallback((textObj: { en: string; ar: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchPostData = async () => {
      try {
        ...
        const validation = validateBlogContent(getText(postData.content), locale);
  ...
  const handleAddComment = async (content: string, parentId?: string): Promise<void> => {
    if (!commentRateLimiter.isAllowed('comment-submission')) {
      throw new Error(locale === 'ar' ? 'كثرة التعليقات. انتظر قليلاً.' : 'Too many comments. Please wait a moment.');
    }
    ...
    try {
      ...
      if (errorMessage === 'UNAUTHORIZED') {
        throw new Error(locale === 'ar' ? 'يجب تسجيل الدخول للتعليق' : 'Please login to comment');
      }
      throw err;
    } finally {
      ...
      <div className="min-h-screen bg-white">
        ...
        <nav className="bg-gray-50 border-b border-gray-200" aria-label="Breadcrumb">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <ol className="flex items-center space-x-2 text-sm text-gray-500" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
              <li>
            ...
            <div className="text-center mb-12">
              ...
              <div className="flex items-center justify-center gap-8 mb-8 flex-wrap">
                ...
                <div className="flex items-center gap-6 text-sm text-gray-500">
                  <div className="flex items-center gap-2" title={locale === 'ar' ? 'المشاهدات' : 'Views'}>
                  ...
                  <div className="flex items-center gap-2" title={locale === 'ar' ? 'الإعجابات' : 'Likes'}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>
                    <span>{likesCount}</span>
                  </div>
                  <div className="flex items-center gap-2" title={locale === 'ar' ? 'التعليقات' : 'Comments'}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div className="flex flex-wrap gap-3">
              {post.tags.map((tag) => (
                <span
                  ...
                  tabIndex={0}
                  role="button"
                  aria-label={`${locale === 'ar' ? 'موضوع' : 'Topic'}: ${getText(tag.name)}`}
                >
                  {getText(tag.name)}
                </span>
              ))}
          ...
          <div className="mt-12 pt-12 border-t border-gray-100">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <button 
                ...
                aria-label={`${isLiked ? (locale === 'ar' ? 'إلغاء الإعجاب' : 'Unlike') : (locale === 'ar' ? 'أعجبني' : 'Like')} - ${likesCount} ${locale === 'ar' ? 'إعجاب' : 'likes'}`}
              >
              ...
              <button 
                ...
                aria-label={`${locale === 'ar' ? 'مشاركة على فيسبوك' : 'Share on Facebook'}: ${getText(post.title)}`}
              >
              ...
              <button 
                ...
                aria-label={`${locale === 'ar' ? 'مشاركة على تويتر' : 'Share on Twitter'}: ${getText(post.title)}`}
              >
              ...
              <button 
                ...
                aria-label={`${locale === 'ar' ? 'مشاركة على لينكد إن' : 'Share on LinkedIn'}: ${getText(post.title)}`}
              >
        ...
        {relatedPosts.length > 0 && (
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 border-t border-gray-100" aria-label={locale === 'ar' ? 'مقالات ذات صلة' : 'Related articles'}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedPosts.map((relatedPost) => (
                <Link
                  ...
                  aria-label={`${locale === 'ar' ? 'اقرأ المقال' : 'Read article'}: ${getText(relatedPost.title)}`}
                >
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="w-1 h-1 bg-gray-400 rounded-full" aria-hidden="true"></span>
                      <time className="text-xs text-gray-500" dateTime={relatedPost.publishedAt}>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
          <div className="text-center">
            ...
            >
              <svg className={`w-5 h-5 ${locale === 'ar' ? 'ml-2 rotate-180' : 'mr-2'} transition-transform group-hover:-translate-x-1`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
