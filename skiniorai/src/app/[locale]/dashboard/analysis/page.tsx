          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-lg text-gray-600 ${isRTL ? 'font-cairo' : ''}`}>
                {isRTL ? 'تتبع تقدم بشرتك عبر الزمن' : 'Track your skin progress over time'}
              </p>
            </div>
            <Link
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="text-center">
                <p className="text-3xl font-semibold text-gray-900 mb-2">{analyses.length}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="text-center">
                <p className="text-3xl font-semibold text-purple-600 mb-2">
                  {analyses.filter(a => a.type === 'Complete Analysis').length}
                </p>
                <p className={`text-sm text-gray-600 ${isRTL ? 'font-cairo' : ''}`}>
              ...
              >
                ...
                <div className="p-6 border-b border-gray-100">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${getScoreColor(analysis.score)} ${isRTL ? 'font-cairo' : ''}`}>
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    ...
                    <span className="text-sm font-medium text-gray-900">{analysis.skinType}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {analysis.concerns.map((concern, i) => (
                      <span 
                        key={i}
                        className={`px-3 py-1 bg-red-100 text-red-700 text-xs rounded-full ${isRTL ? 'font-cairo' : ''}`}
                      >
                        {concern}
                      </span>
                    ))}
                ...
                {selectedAnalysis === analysis.id && (
                  <div className="p-6 space-y-6">
                    ...
                    {/* Skin Metrics */}
                    <div>
                      <h4 className={`text-md font-semibold text-gray-900 mb-4 ${isRTL ? 'font-cairo' : ''}`}>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {analysis.improvements.map((metric, i) => (
                          <div key={i} className="bg-gray-50 rounded-xl p-4">
                            ...
                            <p className="text-sm text-gray-600">{metric.score}/100</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {analysis.recommendations.map((rec, i) => (
                          <div key={i} className="flex items-start space-x-3 p-3 bg-blue-50 rounded-xl">
                            <div className="w-5 h-5 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                              <span className="text-xs font-medium text-blue-600">{i + 1}</span>
                            </div>
                            <p className={`text-sm text-gray-700 ${isRTL ? 'font-cairo' : ''}`}>
                              {rec}
                            </p>
                          </div>
                        ))}
            ...
            <div className="h-64 bg-gray-50 rounded-xl flex items-center justify-center">
              <div className="text-center">
                ...
                <p className={`text-gray-500 ${isRTL ? 'font-cairo' : ''}`}>
                  {isRTL ? 'رسم بياني لتقدم البشرة' : 'Interactive progress chart coming soon'}
                </p>
              </div>
            </div>
          </div>
