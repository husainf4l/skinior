...
export default function ProductsPage() {
  ...
  const tabs = [
    { key: 'favorites', label: isRTL ? 'المفضلة' : 'Favorites', count: favoriteProducts.length },
    { key: 'recommended', label: isRTL ? 'موصى بها' : 'Recommended', count: recommendedProducts.length },
    { key: 'routine', label: isRTL ? 'الروتين' : 'My Routine', count: favoriteProducts.filter(p => p.inRoutine).length }
  ];

  const getFilteredProducts = () => {
    let products = selectedTab === 'favorites' ? favoriteProducts : 
                   selectedTab === 'recommended' ? recommendedProducts : 
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-lg text-gray-600 ${isRTL ? 'font-cairo' : ''}`}>
                {isRTL ? 'إدارة منتجاتك المفضلة والموصى بها' : 'Manage your favorite and recommended products'}
              </p>
            </div>
            <Link
              href={`/${locale}/shop`}
              className={`bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-colors ${isRTL ? 'font-cairo' : ''}`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="text-center">
                <p className="text-3xl font-semibold text-blue-600 mb-2">{favoriteProducts.length}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="text-center">
                <p className="text-3xl font-semibold text-purple-600 mb-2">{recommendedProducts.length}</p>
                <p className={`text-sm text-gray-600 ${isRTL ? 'font-cairo' : ''}`}>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="text-center">
                <p className="text-3xl font-semibold text-orange-600 mb-2">
                  ${favoriteProducts.reduce((sum, p) => sum + p.price, 0).toFixed(0)}
                </p>
                <p className={`text-sm text-gray-600 ${isRTL ? 'font-cairo' : ''}`}>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className={`text-lg font-semibold text-gray-900 mb-1 ${isRTL ? 'font-cairo' : ''}`}>
                        {product.name}
                      </h3>
                      <p className={`text-sm text-gray-600 mb-2 ${isRTL ? 'font-cairo' : ''}`}>
                        {product.brand}
                      </p>
                  ...
                  <div className="flex items-center space-x-2 mb-3">
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {product.benefits.slice(0, 3).map((benefit, i) => (
                      <span key={i} className={`px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full ${isRTL ? 'font-cairo' : ''}`}>
                        {benefit}
                      </span>
                    ))}
                  </div>
                  ...
                  {selectedTab === 'recommended' && 'matchReason' in product && (
                    <div className="mb-4 p-3 bg-green-50 rounded-xl">
                      <div className="flex items-center space-x-2 mb-1">
                        <svg className="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className={`text-xs font-medium text-green-700 ${isRTL ? 'font-cairo' : ''}`}>
                          {(product as any).confidence}% {isRTL ? 'توافق' : 'Match'}
                        </span>
                      </div>
                      <p className={`text-xs text-green-700 ${isRTL ? 'font-cairo' : ''}`}>
                        {(product as any).matchReason}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedTab !== 'recommended' && 'lastUsed' in product && (
                    <div className="flex items-center justify-between mb-4">
                      <span className={`text-xs text-gray-500 ${isRTL ? 'font-cairo' : ''}`}>
                        {isRTL ? 'آخر استخدام: ' : 'Last used: '}{formatDate((product as any).lastUsed)}
                      </span>
                      {'inRoutine' in product && (product as any).inRoutine && (
                        <span className={`px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full ${isRTL ? 'font-cairo' : ''}`}>
                  ...
                  <div className="flex space-x-2">
                    {selectedTab === 'recommended' ? (
                      <>
                        <button className={`flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-xl font-medium transition-colors ${isRTL ? 'font-cairo' : ''}`}>
                          {isRTL ? 'إضافة للمفضلة' : 'Add to Favorites'}
                        </button>
                        <button className={`px-4 py-2 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-colors ${isRTL ? 'font-cairo' : ''}`}>
                          {isRTL ? 'التفاصيل' : 'Details'}
                        </button>
                      </>
                    ) : (
                      <>
                        <button className={`flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-xl font-medium transition-colors ${isRTL ? 'font-cairo' : ''}`}>
          ...
          {getFilteredProducts().length === 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
              ...
              <p className={`text-gray-500 mb-6 ${isRTL ? 'font-cairo' : ''}`}>
                {isRTL ? 'لم نجد أي منتجات تطابق الفلتر المحدد' : 'No products match the selected filters'}
              </p>
              <Link
                href={`/${locale}/shop`}
                className={`bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-colors ${isRTL ? 'font-cairo' : ''}`}
