...
export default function TableOfContents({
  ...
  useEffect(() => {
    const generateTOC = () => {
      ...
      const headings = contentElement.querySelectorAll(headingSelector) as NodeListOf<HTMLElement>;
      const items: TOCItem[] = [];
      ...
      headings.forEach((heading, index) => {
        ...
        if (!heading.id) {
          ...
          heading.id = `heading-${index}-${text.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`;
        }
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Auto-hide logic
      if (autoHide) {
        const shouldShow = window.scrollY > 300 && tocItems.some(item => 
          item.element.getBoundingClientRect().bottom > 0
        );
  ...
  return (
    <div className={`fixed right-8 top-1/2 transform -translate-y-1/2 w-64 bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden z-40 hidden xl:block ${className}`}>
      {/* Header */}
        ...
        {showProgress && (
          <div className="mt-2 w-full bg-gray-200 rounded-full h-1">
            <div 
              ...
              style={{ 
                width: `${(Object.keys(readingProgress).length / tocItems.length) * 100}%` 
              }}
            />
          </div>
        )}
...
export function FloatingTOCButton({ isVisible, onClick, locale = 'en' }: FloatingTOCButtonProps) {
