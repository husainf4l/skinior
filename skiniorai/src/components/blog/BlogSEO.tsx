...
export default function BlogSEO({ 
  ...
  const generatePostStructuredData = (post: BlogPost) => {
    return {
      ...
      "author": {
        ...
        "url": post.author.socialLinks?.website || `${baseUrl}/author/${post.author.id}`,
        "sameAs": [
      ...
      "wordCount": getText(post.content).replace(/<[^>]*>/g, '').split(' ').length,
  ...
  const generateFAQData = (post: BlogPost) => {
    ...
    const qaPairs = content.match(/<h[2-6][^>]*>([^<]*\?[^<]*)<\/h[2-6]>\s*<p>([^<]*)<\/p>/gi);
    
    if (qaPairs && qaPairs.length > 0) {
      qaPairs.forEach(pair => {
        const questionMatch = pair.match(/<h[2-6][^>]*>([^<]*\?[^<]*)<\/h[2-6]>/);
  ...
  if (isPostPage && post) {
    ...
    return (
      <Head>
        ...
        <meta name="keywords" content={post.tags.map(tag => getText(tag.name)).join(', ')} />
        ...
        <meta property="article:section" content={getText(post.category.name)} />
        {post.tags.map((tag, index) => (
          <meta key={index} property="article:tag" content={getText(tag.name)} />
        ))}
        ...
        <link rel="alternate" href={`${baseUrl}/en/blog/${post.id}`} hrefLang="en" />
        <link rel="alternate" href={`${baseUrl}/ar/blog/${post.id}`} hrefLang="ar" />
  ...
  const title = locale === 'ar' ? 'مدونة سكينيور - نصائح العناية بالبشرة' : 'Skinior Blog - Skincare Tips & Insights';
  ...
  return (
    <Head>
      ...
      <title>{title}</title>
      <meta name="description" content={description} />
      <meta name="keywords" content={locale === 'ar' ? 'العناية بالبشرة, نصائح, مدونة, سكينيور' : 'skincare, tips, blog, beauty, skin health, Skinior'} />
