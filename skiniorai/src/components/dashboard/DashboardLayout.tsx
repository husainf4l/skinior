...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    {
      name: isRTL ? 'الاستشارات' : 'Consultations',
    ...
    <div className={`min-h-screen bg-gray-50 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 flex z-50 lg:hidden">
          ...
          />
          <div className={`relative flex-1 flex flex-col max-w-xs w-full bg-white/95 backdrop-blur-xl border-r border-gray-100 ${isRTL ? 'mr-auto' : 'ml-auto'}`}>
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-white/95 backdrop-blur-xl border-r border-gray-100">
      ...
      <div className="flex-1 flex flex-col pt-6 pb-4 overflow-y-auto">
        <div className="flex items-center flex-shrink-0 px-6">
          ...
          <span className={`ml-3 text-2xl font-semibold text-gray-900 ${isRTL ? 'mr-3 ml-0 font-cairo' : ''}`}>
            Skinior
          </span>
        </div>
        ...
        <nav className="mt-10 flex-1 px-4 space-y-2">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className={`group flex items-center px-4 py-3 text-sm font-medium rounded-2xl text-gray-600 hover:bg-gray-100/50 hover:text-gray-900 transition-all duration-200 ${isRTL ? 'font-cairo' : ''}`}
            >
              <span className={`mr-3 flex-shrink-0 h-5 w-5 text-gray-400 group-hover:text-gray-600 ${isRTL ? 'ml-3 mr-0' : ''}`}>
                {item.icon}
              </span>
              {item.name}
            </Link>
      ...
      <div className="flex-shrink-0 border-t border-gray-100 p-6">
        <div className="flex items-center p-3 rounded-2xl bg-gray-50/50">
          ...
          <div className={`ml-3 flex-1 ${isRTL ? 'mr-3 ml-0' : ''}`}>
            ...
            <button
              onClick={onLogout}
              className={`text-xs text-gray-500 hover:text-gray-700 transition-colors ${isRTL ? 'font-cairo' : ''}`}
            >
