...
const TodaysDeals = memo(() => {
    ...
    const fetchDeals = async () => {
      try {
        const response = await fetch(
          `${API_CONFIG.API_BASE_URL}/products/deals/today`
        );

        if (!response.ok) {
          console.error(
            `Today's deals API error: ${response.status} ${response.statusText}`
          );
          return;
        }
  ...
  return (
    <section
      className={`py-20 bg-white/70 backdrop-blur-2xl ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p
            className={`mt-4 text-lg text-gray-600 max-w-2xl mx-auto ${
              isRTL ? "font-cairo leading-relaxed" : "leading-relaxed"
            }`}
          >
            {isRTL
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white/80 backdrop-blur-2xl border border-white/20 shadow-2xl shadow-gray-900/10">
          <div
            ...
            style={{
              transform: `translateX(${
                isRTL ? currentDealIndex * 100 : -currentDealIndex * 100
              }%)`,
            }}
          >
                  ...
                  >
                    ...
                    <div className="mb-8">
                      ...
                      >
                        ...
                        >
                          {isRTL
                            ? `${deal.price.toFixed(2)} د.أ`
                            : `JOD ${deal.price.toFixed(2)}`}
                        </span>
                        <span
                        ...
                        >
                          {isRTL
                            ? `${deal.compareAtPrice.toFixed(2)} د.أ`
                            : `JOD ${deal.compareAtPrice.toFixed(2)}`}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      <button
                        onClick={() =>
                          (window.location.href = `/${locale}/products/${deal.id}`)
                        }
                        className={`bg-gray-900 hover:bg-gray-800 text-white px-8 py-4 rounded-xl text-lg font-medium transition-all duration-200 hover:scale-105 ${
                          isRTL ? "font-cairo" : ""
                        }`}
                      >
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${
                    index === currentDealIndex ? "bg-gray-900" : "bg-gray-600"
                  }`}
                />
              </button>
            ))}
          ...
          <button
            onClick={() => {
              const newIndex =
                (currentDealIndex - 1 + deals.length) % deals.length;
