...
async function main() {
  // Create brands
  const skiniorBrand = await prisma.brand.upsert({
    where: { slug: 'skinior' },
    update: {
      name: 'Skinior',
      nameAr: 'سكينيور',
  ...
  const ceraveBrand = await prisma.brand.upsert({
    where: { slug: 'cerave' },
    update: {
      name: 'CeraVe',
      nameAr: 'سيرافي',
  ...
  const ordinaryBrand = await prisma.brand.upsert({
  ...
  const vitaminCSerum = await prisma.product.upsert({
  ...
  const hyaluronicMoisturizer = await prisma.product.upsert({
    where: { slug: 'hyaluronic-acid-moisturizer' },
    update: {
  ...
  const retinolTreatment = await prisma.product.upsert({
    where: { slug: 'retinol-night-treatment' },
    update: {
  ...
  const niacinamidePores = await prisma.product.upsert({
    where: { slug: 'niacinamide-pore-refiner' },
    update: {
  ...
  // Add or refresh product images: delete existing images for each product and recreate
  const productsToSeed = [
    { product: vitaminCSerum, images: [
      { url: 'https://placehold.co/600x900', altText: 'Vitamin C Serum', isMain: true, sortOrder: 1 },
      { url: 'https://placehold.co/600x900/000000/FFF', altText: 'Vitamin C Serum Secondary', isHover: true, sortOrder: 2 },
    ]},
    { product: hyaluronicMoisturizer, images: [
      { url: 'https://placehold.co/600x900', altText: 'Hyaluronic Moisturizer', isMain: true, sortOrder: 1 },
      { url: 'https://placehold.co/600x900/000000/FFF', altText: 'Hyaluronic Moisturizer Secondary', isHover: true, sortOrder: 2 },
    ]},
    { product: retinolTreatment, images: [
      { url: 'https://placehold.co/600x900', altText: 'Retinol Treatment', isMain: true, sortOrder: 1 },
      { url: 'https://placehold.co/600x900/000000/FFF', altText: 'Retinol Treatment Secondary', isHover: true, sortOrder: 2 },
    ]},
    { product: niacinamidePores, images: [
      { url: 'https://placehold.co/600x900', altText: 'Niacinamide Pore Refiner', isMain: true, sortOrder: 1 },
      { url: 'https://placehold.co/600x900/000000/FFF', altText: 'Niacinamide Pore Refiner Secondary', isHover: true, sortOrder: 2 },
    ]},
  ];

  for (const entry of productsToSeed) {
    // delete existing images for product to avoid duplicates
    await prisma.productImage.deleteMany({ where: { productId: entry.product.id } });
    // recreate images
    await prisma.productImage.createMany({
      data: entry.images.map(img => ({
  ...
  // Add some reviews only if none exist for product
  const reviewSeeds = [
    { productId: vitaminCSerum.id, customerName: 'Sarah M.', rating: 5, title: 'Amazing results!', comment: 'My skin looks brighter after just 2 weeks of use.', isVerified: true },
    { productId: hyaluronicMoisturizer.id, customerName: 'Ahmed K.', rating: 4, title: 'Very hydrating', comment: 'Great moisturizer for dry skin.', isVerified: true },
    { productId: retinolTreatment.id, customerName: 'Layla H.', rating: 5, title: 'Effective anti-aging', comment: 'Noticed improvement in fine lines.', isVerified: true },
  ];

  for (const r of reviewSeeds) {
    const count = await prisma.productReview.count({ where: { productId: r.productId } });
    if (count === 0) {
