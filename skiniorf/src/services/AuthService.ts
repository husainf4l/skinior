...
class AuthService {
  ...
  async googleSignIn(): Promise<AuthResponse> {
    try {
      ...
      // Use mobile-specific endpoint with simplified data format
      const mobileAuthData = {
        idToken: googleAuthData.idToken,
        email: googleAuthData.user.email,
        firstName: googleAuthData.user.name.split(' ')[0] || '',
        lastName: googleAuthData.user.name.split(' ').slice(1).join(' ') || ''
      };

      const fullUrl = `${apiBaseUrl}/auth/google/mobile`;
      console.log('Google Sign-In URL:', fullUrl);
      console.log('Google Sign-In Request Data:', JSON.stringify(mobileAuthData, null, 2));
      ...
      const requestOptions = {
        ...
        body: JSON.stringify(mobileAuthData),
      };

      console.log('Google Sign-In Full Request:', {
        url: fullUrl,
  ...
  async appleSignIn(appleAuthData: AppleAuthData): Promise<AuthResponse> {
    try {
      // Prepare the data for mobile-specific endpoint
      const mobileAuthData = {
        identityToken: appleAuthData.identityToken,
        email: appleAuthData.user?.email || '',
        firstName: appleAuthData.user?.name?.firstName || '',
        lastName: appleAuthData.user?.name?.lastName || ''
      };

      const fullUrl = `${apiBaseUrl}/auth/apple/mobile`;
      console.log('Apple Sign-In URL:', fullUrl);
      console.log('Apple Sign-In Request Data:', JSON.stringify(mobileAuthData, null, 2));
  ...
  async refreshToken(): Promise<string | null> {
    try {
      ...
      // Extract token from either new format (tokens.accessToken) or old format (token)
      ...
      const storageData: Array<[string, string]> = [
        [this.tokenKey, newAccessToken],
      ];
      
      if (newRefreshToken) {
        storageData.push([this.refreshTokenKey, newRefreshToken]);
      }
  ...
  private async storeTokens(authData: AuthResponse): Promise<void> {
    ...
    if (refreshToken) {
      storageData.push([this.refreshTokenKey, refreshToken]);
    }
